...
import { updateMaxDiscount } from "../src/methods/admin.js";
...
describe("updateMaxDiscount", () => {
  ...
  it("throws if simulation returns an error", async () => {
    ...
    await expect(
      updateMaxDiscount(mockServer, VALID_CONTRACT, 2000, mockAccount, mockSign, "passphrase")
    ).rejects.toThrow();
  });
  ...
  it("returns txHash on success", async () => {
    ...
    const result = await updateMaxDiscount(
      mockServer,
      VALID_CONTRACT,
      2000,
      mockAccount,
